...
class ClusteredServiceConfiguration
{
public:
  /**
   ...
   */
   static constexpr std::int64_t MARK_FILE_UPDATE_INTERVAL_NS = 1000;

  /**
   * Timeout in milliseconds to detect liveness.
   */
  static constexpr std::int64_t LIVENESS_TIMEOUT_MS = 10 * 1000;

  /**
   * Property name for the identity of the cluster instance.
   */
  static constexpr const char *CLUSTER_ID_PROP_NAME = "aeron.cluster.id";

  /**
   * Default identity for a clustered instance.
   */
  static constexpr std::int32_t CLUSTER_ID_DEFAULT = 0;

  /**
   * Identity for a clustered service. Services should be numbered from 0 and be contiguous.
   */
  static constexpr const char *SERVICE_ID_PROP_NAME = "aeron.cluster.service.id";

  /**
   * Default identity for a clustered service.
   */
  static constexpr std::int32_t SERVICE_ID_DEFAULT = 0;

  /**
   * Name for a clustered service to be the role of the {@link Agent}.
   */
  static constexpr const char *SERVICE_NAME_PROP_NAME = "aeron.cluster.service.name";

  /**
   * Name for a clustered service to be the role of the {@link Agent}.
   */
  static constexpr const char *SERVICE_NAME_DEFAULT = "clustered-service";

  /**
   * Class name for dynamically loading a {@link ClusteredService}. This is used if
   * {@link Context#clusteredService()} is not set.
   */
  static constexpr const char *SERVICE_CLASS_NAME_PROP_NAME = "aeron.cluster.service.class.name";

  /**
   * Channel to be used for log or snapshot replay on startup.
   */
  static constexpr const char *REPLAY_CHANNEL_PROP_NAME = "aeron.cluster.replay.channel";

  /**
   * Default channel to be used for log or snapshot replay on startup.
   */
  static constexpr const char *REPLAY_CHANNEL_DEFAULT = "ipc";

  /**
   * Stream id within a channel for the clustered log or snapshot replay.
   */
  static constexpr const char *REPLAY_STREAM_ID_PROP_NAME = "aeron.cluster.replay.stream.id";

  /**
   * Default stream id for the log or snapshot replay within a channel.
   */
  static constexpr std::int32_t REPLAY_STREAM_ID_DEFAULT = 103;

  /**
   * Channel for control communications between the local consensus module and services.
   */
  static constexpr const char *CONTROL_CHANNEL_PROP_NAME = "aeron.cluster.control.channel";

  /**
   * Default channel for communications between the local consensus module and services. This should be IPC.
   */
  static constexpr const char *CONTROL_CHANNEL_DEFAULT = "aeron:ipc?term-length=128k";

  /**
   * Stream id within the control channel for communications from the consensus module to the services.
   */
  static constexpr const char *SERVICE_STREAM_ID_PROP_NAME = "aeron.cluster.service.stream.id";

  /**
   * Default stream id within the control channel for communications from the consensus module.
   */
  static constexpr std::int32_t SERVICE_STREAM_ID_DEFAULT = 104;

  /**
   * Stream id within the control channel for communications from the services to the consensus module.
   */
  static constexpr const char *CONSENSUS_MODULE_STREAM_ID_PROP_NAME = "aeron.cluster.consensus.module.stream.id";

  /**
   * Default stream id within a channel for communications from the services to the consensus module.
   */
  static constexpr std::int32_t CONSENSUS_MODULE_STREAM_ID_DEFAULT = 105;

  /**
   * Channel to be used for archiving snapshots.
   */
  static constexpr const char *SNAPSHOT_CHANNEL_PROP_NAME = "aeron.cluster.snapshot.channel";

  /**
   * Default channel to be used for archiving snapshots.
   */
  static constexpr const char *SNAPSHOT_CHANNEL_DEFAULT = "aeron:ipc?alias=snapshot";

  /**
   * Stream id within a channel for archiving snapshots.
   */
  static constexpr const char *SNAPSHOT_STREAM_ID_PROP_NAME = "aeron.cluster.snapshot.stream.id";

  /**
   * Default stream id for the archived snapshots within a channel.
   */
  static constexpr std::int32_t SNAPSHOT_STREAM_ID_DEFAULT = 106;

  /**
   * Directory to use for the aeron cluster.
   */
  static constexpr const char *CLUSTER_DIR_PROP_NAME = "aeron.cluster.dir";

  /**
   * Directory to use for the Cluster component's mark file.
   */
  static constexpr const char *MARK_FILE_DIR_PROP_NAME = "aeron.cluster.mark.file.dir";

  /**
   * Default directory to use for the aeron cluster.
   */
  static constexpr const char *CLUSTER_DIR_DEFAULT = "aeron-cluster";

  /**
   * Length in bytes of the error buffer for the cluster container.
   */
  static constexpr const char *ERROR_BUFFER_LENGTH_PROP_NAME = "aeron.cluster.service.error.buffer.length";

  /**
   * Default length in bytes of the error buffer for the cluster container.
   */
  static constexpr std::int32_t ERROR_BUFFER_LENGTH_DEFAULT = 1024 * 1024;

  /**
   * Is this a responding service to client requests property.
   */
  static constexpr const char *RESPONDER_SERVICE_PROP_NAME = "aeron.cluster.service.responder";

  /**
   * Default to true that this a responding service to client requests.
   */
  static constexpr bool RESPONDER_SERVICE_DEFAULT = true;

  /**
   * Fragment limit to use when polling the log.
   */
  static constexpr const char *LOG_FRAGMENT_LIMIT_PROP_NAME = "aeron.cluster.log.fragment.limit";

  /**
   * Default fragment limit for polling log.
   */
  static constexpr std::int32_t LOG_FRAGMENT_LIMIT_DEFAULT = 50;

  /**
   * Delegating {@link ErrorHandler} which will be first in the chain before delegating to the
   * {@link Context#errorHandler()}.
   */
  static constexpr const char *DELEGATING_ERROR_HANDLER_PROP_NAME =
   ...
   * for being exceeded.
   */
  static constexpr const char *CYCLE_THRESHOLD_PROP_NAME = "aeron.cluster.service.cycle.threshold";

  /**
   * Default threshold value for the container work cycle threshold to track for being exceeded.
   */
  static constexpr std::int64_t CYCLE_THRESHOLD_DEFAULT_NS = 1000000;

  /**
   * Counter type id for the cluster node role.
   */
  // TODO
  //static constexpr std::int32_t CLUSTER_NODE_ROLE_TYPE_ID = AeronCounters.CLUSTER_NODE_ROLE_TYPE_ID;

  /**
   * Counter type id of the commit position.
   */
  // TODO
  //static std::int32_tCOMMIT_POSITION_TYPE_ID = AeronCounters.CLUSTER_COMMIT_POSITION_TYPE_ID;

  /**
   * Counter type id for the clustered service error count.
   */
  // TODO
  //static std::int32_t CLUSTERED_SERVICE_ERROR_COUNT_TYPE_ID =
